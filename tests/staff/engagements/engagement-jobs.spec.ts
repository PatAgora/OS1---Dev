/**
 * Staff Portal — Engagement Jobs (/engagement/<id>/jobs)
 *
...
test('/engagement/<id>/jobs — page loads', async ({ page }) => {
  const id = await getFirstEngagementId(page);
  const response = await page.goto(`/engagement/${id}/jobs`, { waitUntil: 'domcontentloaded' });
